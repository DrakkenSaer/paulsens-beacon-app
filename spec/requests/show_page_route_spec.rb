...
RSpec.describe "Show Pages Route", :type => :request do
  ...
  describe "allowed parameter strings" do
    before :each do
       ...
       5.times { |i| FactoryGirl.create(:historical_event, title: "new #{i}", description: "description #{i}", date: Date.today - i.days) }
    end
    
    it "assigns veriable if parameter string is only model name" do
      get root_path
      expect(assigns(:historical_events).count).to eql HistoricalEvent.count
    end
    
    #Remaining tests do not work with routes spec as is
    
    # it "allows use of limit" do
    #   get root_path(resources: {historical_events: "HistoricalEvent.limit(2)"})
    #   expect(assigns(:historical_events).count).to eql 2
    #   expect(assigns(:historical_events).count).to_not eql HistoricalEvent.count
    # end
